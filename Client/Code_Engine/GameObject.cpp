...
void CGameObject::CreateObjectConstantBuffer()
{
	//int elementCount = 1;
...
void CGameObject::CreatePassConstantBuffer()
{
	m_ElementByteSize_pass = BufferUtility::CalcConstantBufferByteSize(sizeof(PassConstants));

	ThrowIfFailed(m_DxDevice->GetDevice().Get()->CreateCommittedResource(
		&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
		D3D12_HEAP_FLAG_NONE,
		&CD3DX12_RESOURCE_DESC::Buffer(m_ElementByteSize_pass),
		D3D12_RESOURCE_STATE_GENERIC_READ,
		nullptr,
		IID_PPV_ARGS(&m_UploadBuffer_pass)));

	ThrowIfFailed(m_UploadBuffer_pass->Map(0, nullptr, reinterpret_cast<void**>(&m_MappedData_pass)));
}

void CGameObject::Init_Start()
{
	ThrowIfFailed(m_DxDevice->GetCommandList()->Reset(m_DxDevice->GetCommandAllocator().Get(), nullptr));
}

void CGameObject::Init_End()
{
