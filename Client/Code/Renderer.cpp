...
bool CRenderer::Init_Component()
{
	...
	if (!BuildNecessaryModules())
		return false;
	m_DxDevice->SetDeviceInitState(true);
	return true;
}
...
void CRenderer::Render(const std::shared_ptr<CTimer> t)
{
	
}

void CRenderer::Render_Begin()
{
	...
	ThrowIfFailed(m_CommandList->Reset(m_DirectCmdListAlloc.Get(), m_PSOs["opaque"].Get()));
...
//
//void CRenderer::SetClientSize(int width, int height)
//{
//	m_ClientWidth = width;
...
bool CRenderer::Get4xMsaaState() const
{
	return m4xMsaaState;
}
...
void CRenderer::OnResize()
{
	...
	m_ScreenViewport.Width = static_cast<float>(g_ClientWidth);
	m_ScreenViewport.Height = static_cast<float>(g_ClientHeight);
	m_ScreenViewport.MinDepth = 0.0f;
	m_ScreenViewport.MaxDepth = 1.0f;

	m_ScissorRect = { 0, 0, g_ClientWidth, g_ClientHeight };

	//m_ClientResize = false;
...
//
//float CRenderer::AspectRatio() const
//{
...
#ifdef _DEBUG
	m_DxDevice->LogAdapters(m_BackBufferFormat);
#endif
...
bool CRenderer::BuildNecessaryModules()
{
...
//}
//
//void CRenderer::BuildConstantBuffers()
//{
...
void CRenderer::BuildShadersAndInputLayout()
{
	...
	BuildShader_LandWave();
	BuildInputLayout_LandWave();
...
void CRenderer::BuildPSOs()
{
	...
	opaquePsoDesc.InputLayout = { m_InputLayout["LandWave"].data(), (UINT)m_InputLayout["LandWave"].size() };
...
Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList> CRenderer::GetCommandList()
{
	return m_CommandList;
}

ID3D12Resource * CRenderer::CurrentBackBuffer() const
{
	return m_SwapChainBuffer[m_CurrBackBuffer].Get();
}

D3D12_CPU_DESCRIPTOR_HANDLE CRenderer::CurrentBackBufferView() const
{
...
D3D12_CPU_DESCRIPTOR_HANDLE CRenderer::DepthStencilView() const
{
